...
if (!empty($statustext)) {
	...
	if (isset($_GET['la']) and isset($_GET['savejob'])) {
		echo "elseif1";
		$conn->query("update scan2557 set ch" . $date . "='" . date("Y-m-d | H:i:s") . "| (ลา)',status='" . $_GET['status'] . "',savejob='" . $_GET['savejob'] . "',statustext=NULL ,type123='" . $type123 . "'where std_id='" . $_GET['id'] . "';");
		echo "<meta http-equiv='refresh' content='0;url=../menu_1.php'>";
	}
	if (!isset($_GET['la']) and !isset($_GET['savejob'])) {
		echo "elseif2";
		$conn->query("update scan2557 set ch" . $date . "='" . date("Y-m-d | H:i:s") . "',status='" . $_GET['status'] . "',statustext=NULL ,type123='" . $type123 . "'where std_id='" . $_GET['id'] . "';");
		echo "<meta http-equiv='refresh' content='0;url=../menu_1.php'>";
	}
	if (isset($_GET['la']) and !isset($_GET['savejob'])) {
		echo "elseif3";
		$conn->query("update scan2557 set ch" . $date . "='" . date("Y-m-d | H:i:s") . " | (ลา)',status='" . $_GET['status'] . "',statustext=NULL ,type123='" . $type123 . "'where std_id='" . $_GET['id'] . "';");
		echo "<meta http-equiv='refresh' content='0;url=../menu_1.php'>";
	}
	if (!isset($_GET['la']) and isset($_GET['savejob'])) {
		echo "elseif4";
		$conn->query("update scan2557 set ch" . $date . "='" . date("Y-m-d | H:i:s") . "',status='" . $_GET['status'] . "',savejob='" . $_GET['savejob'] . "',statustext=NULL ,type123='" . $type123 . "'where std_id='" . $_GET['id'] . "';");
		echo "<meta http-equiv='refresh' content='0;url=../menu_1.php'>";
	}
	$conn->close();
	//echo "<center><h4>System Update...</h4></center>";
